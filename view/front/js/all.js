...
// Liste des événements pour l'exemple
const events = [
    { day: 5, month: 3, year: 2024, name: 'Concert', image: 'event1.jpg' },
    { day: 12, month: 3, year: 2024, name: 'Exposition', image: 'event2.jpg' },
];
...
function generateCalendar(month, year) {
    ...
    let firstDay = new Date(year, month).getDay(); // Jour de la semaine pour le 1er jour du mois
    let daysInMonth = new Date(year, month + 1, 0).getDate(); // Nombre de jours dans le mois

    // Effacer le calendrier précédent
    calendarBody.innerHTML = '';  

    // Créer des cases vides jusqu'au premier jour du mois
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (hasEvent(day, month + 1, year)) {
            cell.classList.add('event-day');
            cell.setAttribute('data-date', day); // Ajouter l'attribut data pour l'événement
        }

        // Ajouter un événement click pour afficher les détails
        cell.addEventListener('click', function () {
            let date = this.getAttribute('data-date');
            showEventDetails(date, month + 1, year);
        });
...
// Trouver un événement pour un jour spécifique
function findEvent(day, month, year) {
    return events.find(event => event.day === day && event.month === month && event.year === year);
}
